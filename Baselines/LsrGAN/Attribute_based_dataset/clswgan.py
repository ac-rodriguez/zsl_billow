...
parser.add_argument('--valsplit', default='1hop', choices=['1hop','2hop','3hop','4hop','allhop'], help='only for inaturalist2017 dataset')
...
if opt.manualSeed is None or opt.manualSeed == -1:
...
if opt.dataset == "AWA1":
    ...
    # save_directory_path =  "GZSL:" + str(opt.unseen_cls_weight) + "_" + str("out_test") + "_" + str(opt.dataset)  + "_" + str(opt.epsilon) + "_" + str(opt.correlation_penalty) + "_" + str(opt.unseen_start) + '_' + str(opt.CENT_LAMBDA)  + '_' + str(opt.class_embedding)
    save_directory_path =  f"GZSL_{opt.unseen_cls_weight}_out_test_{opt.dataset}_{opt.epsilon}_{opt.correlation_penalty}_{opt.unseen_start}_{opt.CENT_LAMBDA}_{opt.class_embedding}"
else:
...
save_directory_path = os.path.join(opt.outf,save_directory_path+ '/version')
...
# load data
if opt.dataset == 'inat2017':
    train_data = dataloader_inat.H5Dataset_inat2017(split='train', opt=opt, is_load_in_memory=True)
    val_seen_data = dataloader_inat.H5Dataset_inat2017(split='val_seen', opt=opt,is_load_in_memory=True)
    val_unseen_data = dataloader_inat.H5Dataset_inat2017(split='val_'+opt.valsplit, opt=opt,is_load_in_memory=True)
...
if opt.dataset == 'inat2017':
    pretrain_cls = classifier.CLASSIFIER(data.train_feature, data.train_label, opt.nclass_all, opt.resSize, opt.cuda, 0.001, 0.5, 50, 100, opt.pretrain_classifier, topk=True, index_per_hop=data.test_unseen_indexperhop)
else:
...
for epoch in tqdm.trange(opt.nepoch):
    ...
    # log_text = 'This is for the seen training [%d/%d] Loss_D: %.4f Loss_G: %.4f, Wasserstein_dist: %.4f, c_errG:%.4f, E_dis : %.4f, Corr_Loss : %4f' %
    #  (epoch, opt.nepoch, D_cost.item(), G_cost.item(), Wasserstein_D.item(), c_errG.item(), Euclidean_loss.item(), Correlation_loss.item()) 
    ...
    if epoch > opt.unseen_start and opt.correlation_penalty > 0:
        if opt.gzsl:
            ...
            writer.add_scalar('unseen_training/c_errG_zero',c_errG_zero.item(), global_step=epoch)    
            writer.add_scalar('unseen_training/Corr_Loss',Correlation_loss_zero.item(), global_step=epoch)    
            writer.add_scalar('unseen_training/Cls_Org',c_errG_orginal.item(), global_step=epoch)    
    ...
    if opt.gzsl:
        if opt.no_classifier == True:
            if opt.dataset == 'inat2017':
                ...
                counter = counter + 1    
                metrics_seen = pretrain_cls.val_gzsl_topk(data.test_seen_feature, data.test_seen_label, data.seenclasses, prefix_m='seen_')
                metrics_unseen = pretrain_cls.val_gzsl_topk(data.test_unseen_feature, data.test_unseen_label, data.unseenclasses,prefix_m='unseen_',valhops=True)
                metrics = {**metrics_seen,**metrics_unseen}
                acc_seen, acc_unseen = metrics['seen_top1'], metrics['unseen_top1']
                H = 0 if (acc_seen+acc_unseen) else  2*acc_seen*acc_unseen / (acc_seen+acc_unseen)
                metrics['H'] = H
                print('unseen=%.4f, seen=%.4f, h=%.4f' % (acc_seen, acc_unseen, H))
                ...
                writer.add_scalar('test/unseen_acc',acc_unseen, global_step=epoch)    
                writer.add_scalar('test/seen_acc',acc_seen, global_step=epoch)    
                writer.add_scalar('test/H',H, global_step=epoch)    
