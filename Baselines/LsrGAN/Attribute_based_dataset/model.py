...
class MLP_CRITIC(nn.Module):
    def __init__(self, opt): 
        ...
        if opt.class_embedding == 'att':
            fc1_in_size =  opt.attSize
            self.rdc_text = nn.Identity()
        else:
        # elif 'billow' in opt.class_embedding or 'sent' == opt.class_embedding:
            # 64*32*32
            self.rdc_text = nn.Linear(opt.attSize, self.rdc_text_dim)

            fc1_in_size =  self.rdc_text_dim
        # self.fc1 = nn.Linear(opt.resSize + opt.attSize, opt.ndh)
        self.fc1 = nn.Linear(opt.resSize + fc1_in_size, opt.ndh)
        #self.fc2 = nn.Linear(opt.ndh, opt.ndh)
...
class MLP_G(nn.Module):
    def __init__(self, opt):
        ...
        # elif 'billow' in opt.class_embedding or 'sent' == opt.class_embedding:
            ...
            fc1_in_size =  self.rdc_text_dim
            # self.non_lin_out = nn.Tanh()

        self.fc1 = nn.Linear(fc1_in_size + opt.nz, opt.ngh)
        self.fc2 = nn.Linear(opt.ngh, opt.resSize)
        self.lrelu = nn.LeakyReLU(0.2, True)
        #self.prelu = nn.PReLU()
        self.non_lin_out = nn.ReLU(True)
