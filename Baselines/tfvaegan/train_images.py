...
save_directory_path = util.add_version_path(opt.outf+'/version')
import os
...
for epoch in range(0,opt.nepoch):
    ...
    writer.add_scalar('vae_loss_seen',vae_loss_seen.item(), global_step=epoch)    
    ...
    netF.eval()
    if opt.dataset.startswith('inat'):
        syn_feature, syn_label = generate_syn_feature(netG,data.unseenclasses, data.attribute_unseen, opt.syn_num,netF=netF,netDec=netDec)
        ...
        if opt.gzsl:   
            ...
            gzsl_cls = classifier.CLASSIFIER(train_X, train_Y, data, nclass, opt.cuda, opt.classifier_lr, 0.5, \
                    ...
                    topk=True, index_per_hop=data.test_unseen_indexperhop)

            print('unseen=%.4f, seen=%.4f, h=%.4f' % (gzsl_cls.m['seen_top1'], gzsl_cls.m['seen_top1'], gzsl_cls.m['H']))
            for key, val in gzsl_cls.m.items():
                writer.add_scalar('test/'+key,val, global_step=epoch)
            for key, val in gzsl_cls.best_m.items():
